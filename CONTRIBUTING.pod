...
Be careful to submit the changes are submitted for the C<master> branch, not C<release>.

=item 4.3 Add more changes

You may want to add more changes, or changes may be requested by the reviewer.

Don't hesitate to edit existing commits (C<git commit --amend>) or rebase (C<git rebase -i>)
if necessary. Then force push (C<git push -f>).
