...
This script ease initial environment setup for secure Github exchanges. More
secure that what the Github help pages advice.
...
This tool B<automates the SSH setup>. Less errors due to the human. And
...
SSH security.
(If you loose that computer or if it is compromised, just revoke the key in
...
This gives the flexibility to use B<multiple Github accounts> (and so a
...
(do not impact your other SSH configurations) in F<~/.ssh/config>.
...
As any software that deals with security of your computer or of communications
with others computers (operating system, antivirus, HTTPS implementation,
password storage...), you have to trust in this software. (If you didn't ever
ask yourself that question about the software you already use, you should!).
...
runs is the code in the script. You can audit it (or ask someone in which you
trust to do it for you) to trust it.

=item *

When running, C<github-keygen> only generates files locally on your system. It
will not connect to your Github account (or any other hosts).
...
Once you ran C<github-keygen>, you can (and you should) audit that config to
check the changes it did to your system before connecting to any SSH hosts.

=item *

No parts of that configuration is something that is executable: this is just
some data that OpenSSH will use.
...
is. (But, anyway bug may exists, so read the license before using the software).

=item *

I (Olivier MenguE<eacute>) am not an expert in software security. However this
list should show you that I care enough about security to have thought to many
...
too low. So install some perl >=5.10.1 (either
...
No change in the github-keygen code, but the fatpacked build has been tweaked
